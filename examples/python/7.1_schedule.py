...
def test_dbuf_4wave_mxfp_preshuffle_b_gemm_cpp(
    ...
    options.use_wave_asm_backend = True
    options.dump_intermediates = "build/intermediates"
    options.print_mlir_file = "gemm_mxfp4_dbuf_4wave_preshuffle_b_asm.mlir"
    options.print_mlir = True
    schedule = get_mxfp4_asymmetric_schedule(is_bscale_shuffled=True)
    options.print_ir_after = "all" if is_debug else []
    options = set_default_run_config(options)
...
def test_dbuf_4wave_mxfp_dynamic_mn_preshuffle_b_gemm(
    ...
    """Preshuffle-B MXFP4 GEMM with dynamic M, N."""
    gemm, options = get_tagged_mxfp4_gemm_preshuffle_b(shape, block, wave_shape=(1, 4), reorder_workgroups=True)
    # Make M, N, K dynamic so the compiler does not specialize on problem size.
    ...
    options.use_wave_asm_backend = True
    options.dump_intermediates = "build/intermediates"
    options.print_mlir_file = "gemm_mxfp4_dbuf_4wave_dynamic_mn_preshuffle_b_asm.mlir"
    options.print_mlir = True
    schedule = get_mxfp4_asymmetric_schedule(is_bscale_shuffled=True)
    options.print_ir_after = "all" if is_debug else []
    options = set_default_run_config(options)
    ...
    _run_mxfp_gemm_preshuffle(gemm, shape, all=True)
    print("MXFP GEMM preshuffle-B 4-wave (WaveASM backend) MN test passed!")

def test_dbuf_4wave_mxfp_dynamic_preshuffle_b_gemm_llvm(
    is_debug=False, shape=(1024, 1024, 8192), block=(128, 256, 256)
):
    """Preshuffle-B MXFP4 GEMM with dynamic M, N, K."""
    gemm, options = get_tagged_mxfp4_gemm_preshuffle_b(shape, block, wave_shape=(1, 4))
    ...
    _run_mxfp_gemm_preshuffle(gemm, shape, all=True)
    print("MXFP GEMM preshuffle-B 4-wave (LLVM backend) test passed!")
